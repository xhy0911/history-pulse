...
def pg(title, body, cur):
    return f"<!DOCTYPE html><html lang='zh-CN'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1.0'><title>{esc(title)}</title><style>{CSS}</style></head><body>{nv(cur)}<div class='c'>{body}</div><div class='f'><a href='{URL}'>🌐手机版</a> · 每日更新 · 历史教育学全球资讯</div></body></html>"

log("生成HTML...")
hero = f"<div style='background:linear-gradient(135deg,#1a3520,#2d6a4f);color:#f5e6cc;padding:36px 28px;border-radius:12px;margin-bottom:20px;text-align:center'><div style='font-size:28px;font-weight:800'>🎓 历史教育学 · 全球动态</div><div style='font-size:14px;color:#a8d5ba;margin:6px 0'>{datetime.now().strftime('%Y-%m-%d')} · {len(art)}篇</div></div>"
...
for a in art[:8]:
    ...
    msg += "</div>"
msg += f"<hr style='border:none;border-top:1px solid #ddd;margin:12px 0'><p style='text-align:center'><a href='{URL}' style='display:inline-block;background:#2d6a4f;color:#fff;padding:10px 24px;border-radius:6px;text-decoration:none;font-size:15px'>🌐 点此查看完整网站</a></p><p style='text-align:center;color:#aaa;font-size:12px;margin-top:8px'>每日更新 · 历史教育学全球资讯</p>"
